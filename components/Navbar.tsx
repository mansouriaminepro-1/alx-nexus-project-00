      ...
      <nav className={`fixed w-full z-50 transition-all duration-500 ${scrolled ? 'bg-white/95 backdrop-blur-md py-4 shadow-sm' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center gap-10">
              {['How It Works', 'Features', 'Live Battles', 'FAQ'].map((item) => (
                <a key={item} href={`/#${item.replace(/ /g, '-').toLowerCase()}`} className="text-sm font-bold uppercase tracking-wide text-brand-black hover:text-brand-black/70 transition-colors relative group">
                  {item}
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-white border-t border-gray-100 shadow-xl p-4 flex flex-col gap-4">
            {['How It Works', 'Features', 'Live Battles', 'FAQ'].map((item) => (
              <a key={item} href={`/#${item.replace(/ /g, '-').toLowerCase()}`} className="text-lg font-bold text-brand-black py-2" onClick={() => setIsMenuOpen(false)}>
                {item}
              </a>
            ))}
